...
const HydrationTracker = ({ data }) => {
  ...
  const averageIntake = useMemo(() => {
    const totalLiters = hydrationData.reduce((sum, entry) => sum + entry.amount, 0);
    const daysWithIntake = hydrationData.filter((entry) => entry.amount > 0).length || 1; // Avoid division by 0
  ...
  return (
    ...
    >
      ...
      {!data || data.length === 0 ? (
        <p className="text-gray-600 text-center">No hydration data for this week.</p>
      ) : (
        <>
                ...
                <svg viewBox="0 0 300 100" className="w-full h-full" preserveAspectRatio="none">
                  ...
                  <motion.path
                    d={hydrationData
                      ...
                      .join(' ') + ` L ${(300 / 7) * 6.5},100 L ${(300 / 7) * 0.5},100 Z`}
                  ...
                  <defs>
                    ...
                    <linearGradient id="hydrationStroke" x1="0%" y1="0%" x2="100%" y2="0%">
          ...
          <div className="mt-4 grid grid-cols-7 gap-2">
            {hydrationData.map((data, index) => (
              <div key={index} className="text-center">
                ...
                <div className="text-xs font-medium">{data.amount > 0 ? `${data.amount}L` : "-"}</div>
              </div>
            ))}
          </div>
        </>
